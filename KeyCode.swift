...
/// A small wrapper around the kVK_ ANSI virtual-key constants we actually use.
/// Adding more is one-line — keep the list short and intentional.
enum KeyCode: UInt16, CaseIterable {
    ...
    /// Best-effort human-readable label for an arbitrary virtual key code.
    /// Used by the hotkey recorder to show the user what they bound.
    static func label(forVirtualKey keyCode: UInt32) -> String {
        switch Int(keyCode) {
